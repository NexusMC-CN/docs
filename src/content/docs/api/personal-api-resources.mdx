---
title: 个人 API：资源
description: 查询、创建和更新资源的接口，以及每个请求可直接使用的完整参数表。
section: API
sidebarGroup: 资源接口
order: 10
---
...
<DocsApiTable
  ...
  rows={[
  { method: 'GET', path: '/api/resources/{id}/edit', auth: 'resource:read:self', description: '读取自己的资源编辑数据' },
...
`{id}` 可以使用资源 ID。正文 `content` 的结构见 [投稿正文 TipTap 快速接入](/docs/api/tiptap-content-format/)，文件项见 [资源文件与版本](/docs/api/personal-api-resource-files/)。
...
<DocsEndpoint method="GET" path="/api/resources/{id}/edit" scope="resource:read:self" />

::: code-group

```bash [bash]
curl "https://www.nexusmc.cn/api/resources/RESOURCE_ID/edit" \
  -H "Authorization: Bearer avm_pat_xxxxx_xxxxx" \
  -H "Accept: application/json"
```

```javascript [javascript]
const response = await fetch('https://www.nexusmc.cn/api/resources/RESOURCE_ID/edit', {
  headers: {
    Authorization: `Bearer ${process.env.NEXUSMC_API_TOKEN}`,
    Accept: 'application/json',
  },
...
response = requests.get(
    'https://www.nexusmc.cn/api/resources/RESOURCE_ID/edit',
    headers={
        'Authorization': f"Bearer {os.environ['NEXUSMC_API_TOKEN']}",
        'Accept': 'application/json',
    },
