...
description: files 子结构、版本 Tag、主文件、Loader 继承、版本提交和文件审核切换机制。
section: API
sidebarGroup: 资源接口
order: 20
---
...
版本列表中的每一项还会返回 `versionTag`。`GET /api/resources/{id}/versions` 当前返回全部可见且已通过审核的版本，不接收筛选查询参数；API 客户端可按以下字段在本地筛选：
