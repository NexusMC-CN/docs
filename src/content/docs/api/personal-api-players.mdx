...
<DocsApiTable
  ...
  rows={[
  { method: 'GET', path: '/api/players/{id}/edit', auth: 'player:read:self', description: '读取自己的投稿编辑数据' },
...
正文 `content` 推荐使用 [投稿正文 TipTap 快速接入](/docs/api/tiptap-content-format/)。
...
<DocsEndpoint method="GET" path="/api/players/{id}/edit" scope="player:read:self" />

::: code-group

```bash [bash]
curl "https://www.nexusmc.cn/api/players/PLAYER_ID/edit" \
  -H "Authorization: Bearer avm_pat_xxxxx_xxxxx" \
  -H "Accept: application/json"
```

```javascript [javascript]
const response = await fetch('https://www.nexusmc.cn/api/players/PLAYER_ID/edit', {
  headers: {
    Authorization: `Bearer ${process.env.NEXUSMC_API_TOKEN}`,
    Accept: 'application/json',
  },
...
response = requests.get(
    'https://www.nexusmc.cn/api/players/PLAYER_ID/edit',
    headers={
        'Authorization': f"Bearer {os.environ['NEXUSMC_API_TOKEN']}",
        'Accept': 'application/json',
    },
